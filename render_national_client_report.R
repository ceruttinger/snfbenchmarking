# render_national_client_report.R
# Render a client report for any SNF CCN using national peer/report data.
# Usage:
...
# Build/refresh the stable national public-use metric table if it is missing.
...
Sys.setenv(SNF_TARGET_CCNS = target_ccn, SNF_DASHBOARD_STATE = "ALL", SNF_OUTPUT_PREFIX = "national")
source("08_build_national_report_data.R")

quarto::quarto_render(
  "client_report_snf_facility.qmd",
  execute_params = list(
    target_ccn = target_ccn,
    state_filter = "ALL",
    data_prefix = "national",
    explorer_href = "https://snfbenchmarking.com/"
  )
)

source_html <- "client_report_snf_facility.html"
...
message("National client report rendered: ", out_file)
invisible(out_file)
